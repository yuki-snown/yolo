...
def main():
    ...
    model.to(device)
    data_set = val_Dataset(val_data, anchors, model_size=model_size, cls_num=cls_num)

    model.eval()
    with torch.no_grad():
        for img, tgt in torch.utils.data.DataLoader(data_set, batch_size=1, shuffle=True):
            image = img.to(device)
                ...
                for area in range(0,channels*3, channels):
                    for i in range(output[c].shape[2]):
                        for j in range(output[c].shape[3]):
                            t = tgt[c][0,area+4:area+channels,i,j]
